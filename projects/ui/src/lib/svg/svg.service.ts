...
import { bufferWhen, catchError, concatMap, filter, map, takeUntil, tap } from 'rxjs/operators';
import { BaseObject } from 'projects/container/src/app/_shared/base/base-object';
import { SettingsService } from 'projects/container/src/app/services/api/settings.service';
...
export class SvgService extends BaseObject {
  ...
  constructor(
    ...
    private zone: NgZone,
    private settingsState: SettingsService,
  ) {
    super();
  ...
  private loadSvgIfRequired(src: string): Observable<SVGElement> {
    if (this.svgCache.has(src)) {
      ...
      console.log(this.settingsState.frontPath);
      console.log(src);
      return this.http
        ...
        concatMap((newTasks) => {
          ...
          newTasks.forEach((newTask) => {
            if (!loadTasksMap.has(newTask.src)) {
              loadTasksMap.set(newTask.src, this.loadSvgIfRequired(newTask.src));
            }
          });

          const loadTasks$ = zip(
            ...[...loadTasksMap.entries()].map(([src, obs]) => zip(of(src), obs)),
          );

          const load$ = loadTasks$.pipe(
            tap((loadedTasks) => {
              newTasks.forEach((newTask) => {
                const loadedTask = loadedTasks.find(([src, _]) => src === newTask.src);
